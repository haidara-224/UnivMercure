...
export function SendDemandeDocument() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (data.type_document === "autre" && !data.comment) {
      setErrorsTypeDocument("Veuillez préciser votre demande.");
      return;
    }
    if (data.type_document === "releve de notes" && !data.comment) {
      setErrorsTypeDocument("Veuillez préciser la classe, le département et l'année scolaire.");
      return;
    }
    if (data.type_document === "attestation d'inscription" && !data.comment) {
      setErrorsTypeDocument("Veuillez préciser la classe, le département et l'année scolaire.");
      return;
    }
            ...
            <div>
              <Label className="block text-sm font-medium text-gray-700 mb-2">
                {data.type_document === "autre"
                  ...
                  : "Merci de fournir la classe, le département ou l'année scolaire selon le type de document"}
              </Label>
